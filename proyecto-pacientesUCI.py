...
def suma_de_Datos (regiones):
	...
	for region in regiones:
		suma_datos = 0
		for i in range(len(datosUCI)):
			if region == datosUCI[i][0]:
				numero = datosUCI[i][4] 
				numero.replace("\n","")
				suma_datos += int(datosUCI[i][4])
...
def UCImaxYmin():
	...
	print (datos)
	print ("\n La región con menor y mayor pacientes UCI son :")
	for idx,value in enumerate (regiones) :
		if len (regiones[idx]) == 1:
			print("  "+ regiones[idx]+ "   |   " + value)
		else:
			print (regiones[idx]+ "   |   " + value )
    minimo = min(datos)
    maximo = max(datos)
